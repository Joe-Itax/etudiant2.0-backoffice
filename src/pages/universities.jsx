...
import { Add, Cancel, Delete, Edit, Replay, Save } from "@mui/icons-material";
import {
...
export default function Universities() {
  ...
  useEffect(() => {
    if (university.length > 0) {
      setNumUniversities(university.length);
    }
  }, [setNumUniversities, university]);

  const handleClickUpdateData = async () => {
    setUpdateDataLoading(true);
    try {
      const universitiesUpdated = await getAllUniversities();

      setUniversity(universitiesUpdated.allUniversities);
      setNumUniversities(universitiesUpdated.allUniversities.length);
      setMessageNotif(universitiesUpdated.message);
      setSeverityNotif("success");
    } catch (error) {
      console.log("error: ", error);
      if (error?.response?.data?.allUniversities) {
        setUniversity(error.response.data.allUniversities);
      }
      if (error?.response) {
        setMessageNotif(error.response.data.message);
      } else if (error?.message) {
  ...
  return (
    <div className="users-page">
      ...
      <div className="mb-4 flex flex-wrap justify-center items-center gap-4 text-white">
        <div className="flex justify-center items-center gap-12 bg-violet-800 p-4 rounded-md">
          <div className="flex flex-col justify-center  gap-5">
            <div>
              <span className="font-extrabold text-4xl">{numUniversities}</span>
            </div>
            <div>
